...
output: 
  html_document:
    ...
    theme: paper
    css: ["css/styles.css", "css/clipboard_buttons.css"]
    mathjax: null
    toc: true
    toc_float: true
---
...
```{js, code=xfun::read_utf8("js/clipboard_buttons.js"), echo=FALSE, cache=FALSE}
```
...
[^2]: The GitHub version of the package may occasionally be more up-to-date than the SSC version. You can install **getcensus** from GitHub by running:
```
...
```{r geographies}
options(knitr.kable.NA = "")
read_excel("doc_tables.xlsx", sheet = "geographies") %>%
  rename(" " = 1) %>%
  kbl() %>%
  kable_styling(position = "left",
                bootstrap_options = "striped",
...
```{r geocomponents}
options(knitr.kable.NA = "")
read_excel("doc_tables.xlsx", sheet = "geocomponents") %>%
  rename(" " = 1) %>%
  kbl() %>%
  kable_styling(position = "left",
                bootstrap_options = "striped",
...
```{r keywords}
options(knitr.kable.NA = "")
read_excel("doc_tables.xlsx", sheet = "keywords") %>%
  kbl() %>%
  kable_styling(position = "left",
                bootstrap_options = "striped",
                fixed_thead = TRUE) %>%
...
<img src="images/dialog_box.png" alt="Screenshot of getcensus Stata dialog box" align="center" width="350" style="border:0.75px solid #0C61A4;"/>
...
<a href="https://www.cbpp.org"><img src="https://www.cbpp.org/sites/all/themes/custom/cbpp/logo.png" alt="Center on Budget and Policy Priorities logo" align="center" width="200"/></a>

### License

[Full license](https://github.com/CenterOnBudget/getcensus/blob/master/LICENSE)

### Contributors

**getcensus** is developed and maintained by [Claire Zippel](https://www.cbpp.org/about/our-staff/claire-zippel) and [Matt Saenz](https://www.cbpp.org/about/our-staff/matt-saenz). It was created by [Raheem Chaudhry](https://github.com/raheem03) and [Vincent
