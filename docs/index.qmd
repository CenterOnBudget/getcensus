...
The Census Bureau publishes thousands of ACS data tables on the web at [data.census.gov](https://www.data.cenus.gov). The underlying data is available through the Census Bureau Data Application Programming Interface (API).[^1]
...
An API key is required to access the Census Bureau Data API. Register for a key at <https://api.census.gov/data/key_signup.html>. 

Users must provide their API key to getcensus by specifying the `key()` option, or, to avoid specifying `key()` each time getcensus is used, storing their API key in a global macro named *censuskey* in their [profile.do](https://www.stata.com/support/faqs/programming/profile-do-file/).
